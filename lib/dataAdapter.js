...
export async function getProductos(tenantId) {
    ...
    const platosFormateados = platosRaw.map(p => {
        // Extraer el identificador/ref/slug/nombre de la categoría
        const catRef = typeof p.categoria === 'object' ? p.categoria?._ref : p.categoria;
        const catRefLimpio = String(catRef || '').toLowerCase().trim();
        
        // 🎯 BÚSQUEDA PROFUNDA SENIOR (Compara contra ID, Slug Y TÍTULO en minúsculas)
        const catVinculada = categoriasRaw.find(c => {
            const idMatch = String(c.id || c._id || '').toLowerCase() === catRefLimpio;
            const slugMatch = String(c.slug?.current || c.slug || '').toLowerCase() === catRefLimpio;
            const tituloMatch = String(c.titulo || c.nombre || '').toLowerCase() === catRefLimpio;
            return idMatch || slugMatch || tituloMatch;
        });
        ...
        if (catVinculada) {
            ...
            // 🛡️ Si es un UUID huérfano feo, lo manda a "GENERAL", si es texto legible usa el texto
            const esUuid = /^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i.test(catRef.trim());
            nombreCategoria = esUuid ? "GENERAL" : catRef.replace(/-/g, ' ');
        }
