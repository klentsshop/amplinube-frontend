...
export async function getProductos(tenantId, terminoBusqueda = '') {
   ...
   // 🧠 UMBRAL DE CORTE BLINDADO:
    ...
    const platosFormateados = platosRaw.map(p => {
        ...
        // 🎯 MATCHEO QUIRÚRGICO SENIOR: Mantiene el UUID relacional y extrae el título visible
        ...
        const uuidCategoriaReal = catVinculada ? (catVinculada.id || catVinculada._id) : (p.categoria || p.categoriaId || catRef);
        const tituloCategoriaReal = catVinculada ? (catVinculada.titulo || catVinculada.nombre) : (p.categoriaNombre || "COCINA");
        ...
        return {
            ...
            // 🛡️ ESTRUCTURA RELACIONAL BLINDADA
            categoria: uuidCategoriaReal,                                     // UUID Puro para Filtros en Memoria y DB
            categoriaNombre: String(tituloCategoriaReal).toUpperCase().trim(),// Título para UI y Comandas
            seImprime: catVinculada?.seImprime ?? p.seImprime ?? true,        // Indicador para Estaciones
