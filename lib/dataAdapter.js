...
export async function getProductos(tenantId) {
    ...
    const platosFormateados = platosRaw.map(p => {
        // Encontrar la categoría vinculada en el mismo paquete crudo
        const idCat = p.categoria?._ref;
        const catVinculada = categoriasRaw.find(c => c._id === idCat);

        // Reconstrucción del array de recetas adaptado al esquema nativo de tu CartContext
        ...
        return {
            ...p,
            _id: p._id || p.id,
            // Simula: "categoria": coalesce(categoria->titulo, "COCINA")
            categoria: catVinculada?.titulo ? catVinculada.titulo.toString().toUpperCase().trim() : "COCINA",
            // Simula: "seImprime": coalesce(categoria->seImprime, true)
            seImprime: catVinculada?.seImprime ?? true,
            // Respaldos de inventario documental intactos
