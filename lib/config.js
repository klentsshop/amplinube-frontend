...
export const SYSTEM = {
    name: "Socio POS", 
    version: "1.2.0",
    developer: "Kelly Johanna Rodriguez"
};
...
// 🎯 La Fuente de la Verdad Única
export let SITE_CONFIG = { ...DEFAULT_CONFIG };

// Guardamos una lista de funciones que quieren enterarse cuando cambie el negocio
const suscriptores = new Set();

export const suscribirseAConfig = (callback) => {
    suscriptores.add(callback);
    return () => suscriptores.delete(callback);
};

// 🚀 INYECTOR MULTITENANT REAL
export const inicializarPOS = (datosSanity) => {
    if (!datosSanity) return;

    // Actualizamos el objeto base
    SITE_CONFIG.brand.name = datosSanity.nombre || DEFAULT_CONFIG.brand.name;
    SITE_CONFIG.brand.nit = datosSanity.nit || DEFAULT_CONFIG.brand.nit;
    SITE_CONFIG.brand.address = datosSanity.direccion || DEFAULT_CONFIG.brand.address;
    SITE_CONFIG.brand.phone = datosSanity.telefono || DEFAULT_CONFIG.brand.phone;

    if (datosSanity.colordark) {
        SITE_CONFIG.theme.dark = datosSanity.colordark;
        if (typeof window !== 'undefined') {
            document.documentElement.style.setProperty('--color-dark', datosSanity.colordark);
        }
    }

    // 📢 Le avisamos a React que los datos cambiaron para que fuerce el redibujo
    suscriptores.forEach(callback => callback({ ...SITE_CONFIG }));
};