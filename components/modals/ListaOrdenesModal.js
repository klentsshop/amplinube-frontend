            ...
            <div style={{ background: 'white', padding: 20, borderRadius: 10, width: '90%', maxWidth: 400 }}>
                ...
                <div style={{ maxHeight: '60vh', overflowY: 'auto' }}>
                    {ordenes.length === 0 ? (
                        ...
                        ordenes.map(orden => {
                            const ordenId = orden.id || orden._id;
                            return (
                                <div key={ordenId} style={{ border: '1px solid #ccc', padding: 10, margin: '5px 0', display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderRadius: '5px' }}>
                                    <div>
                                        <strong style={{ textTransform: 'uppercase' }}>{orden.mesa}</strong>
                                        <p style={{ fontSize: '0.8em', color: '#6B7280', margin: 0 }}>👤 {orden.mesero || orden.nombreMesero || 'Caja'}</p>
                                    </div>
                                    <button 
                                        onClick={() => onCargar(ordenId)} 
