...
setVentas(ventasActivas.reverse());
   ...
   const abrirTicketCable = async (venta) => {
        try {
            // 1️⃣ Enviamos el evento de reimpresión a la tabla 'tickets_caja_pendientes' en Supabase
            const res = await fetch('/api/ventas/reimprimir-caja', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({
            ...
            if (res.ok) {
                console.log("🖨️ Pulso enviado al Bridge C# para reimpresión física.");
            } else {
                console.error("Error al enviar solicitud de reimpresión por cable.");
            }
        } catch (err) {
            console.error("⚠️ Error enviando orden al Bridge C#:", err);
        }
    };
