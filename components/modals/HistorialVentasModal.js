...
export default function HistorialVentasModal({ isOpen, onClose, onReimprimir, tenantId }) {
    ...
    }, [isOpen, tenantId]); // Se ejecuta solo al detectar que el modal se abrió
            ...
            <div style={{ backgroundColor: 'white', width: '95%', maxWidth: '500px', maxHeight: '90vh', borderRadius: '12px', overflow: 'hidden', boxShadow: '0 20px 25px -5px rgba(0, 0, 0, 0.1)', display: 'flex', flexDirection: 'column' }}>
                ...
                <div style={{ padding: '20px', flex: 1, overflowY: 'auto', minHeight: 0 }}>
                    ...
                    <div style={{ marginTop: '20px', border: '1px solid #f1f5f9', borderRadius: '8px' }}>
                        {ventas.length > 0 ? (
                            ventas.map(v => (
                                <div key={v._id} style={{ display: 'flex', flexDirection: 'column', padding: '15px', borderBottom: '1px solid #f1f5f9' }}>
                                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', width: '100%' }}>
                                        ...
                                        <div style={{ display: 'flex', gap: '6px' }}>
                                            <button 
                                                onClick={() => setVentaDetalle(ventaDetalle?._id === v._id ? null : v)} 
                                                style={{ backgroundColor: ventaDetalle?._id === v._id ? '#10b981' : '#e2e8f0', color: ventaDetalle?._id === v._id ? 'white' : '#334155', border: 'none', borderRadius: '6px', padding: '8px 12px', fontSize: '0.85rem', fontWeight: 'bold', cursor: 'pointer', display: 'flex', alignItems: 'center' }}
                    ...
                    {/* VISOR EN PANTALLA DE PRODUCTOS */}
                    {ventaDetalle && (
                        <div style={{ marginTop: '15px', padding: '15px', backgroundColor: '#f8fafc', borderRadius: '8px', border: '1px solid #e2e8f0' }}>
                            ...
                            <div style={{ marginTop: '12px', paddingTop: '10px', borderTop: '2px dashed #cbd5e1', display: 'flex', flexDirection: 'column', gap: '4px', fontSize: '0.85rem' }}>
                                <div style={{ display: 'flex', justifyContent: 'space-between', color: '#64748b' }}>
                                    ...
                                    <span>${(Number(ventaDetalle.totalPagado || 0) - Number(ventaDetalle.propinaRecaudada || 0)).toLocaleString()}</span>
                                </div>
                                ...
                                <div style={{ display: 'flex', justifyContent: 'space-between', color: '#1e293b', fontSize: '1rem', fontWeight: '900', paddingTop: '4px', borderTop: '1px solid #e2e8f0' }}>
                                    <span>💰 TOTAL:</span>
                                    <span style={{ color: '#10b981' }}>${Number(ventaDetalle.totalPagado || 0).toLocaleString()}</span>
                                </div>
                            </div>
