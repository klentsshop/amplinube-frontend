...
<div style={{ backgroundColor: 'white', border: '1px solid #F3F4F6', borderRadius: '8px', padding: '5px 12px' }}>
    {reporte?.estadisticas?.topPlatos?.length > 0 ? (
        ...
        <p style={{ fontSize: '0.8em', color: '#9CA3AF', padding: '10px 0' }}>Sin datos de platos</p>
                        ...
                        {/* --- VENTAS POR MESERO --- */}
                        <h4 style={{ margin: '20px 0 8px 0', fontSize: '0.9em', color: '#4B5563', textTransform: 'uppercase' }}>👤 Ventas por Vendedor</h4>
                        {Object.entries(reporte?.porMesero || {}).length > 0 ? (
                        ...
                        ) : <p style={{ fontSize: '0.8em', color: '#9CA3AF' }}>Sin datos de meseros</p>}
                    </>
                )}
            </div>
        </div>
